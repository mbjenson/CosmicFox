#include "TileMapOld.h"




TileMapOld::TileMapOld(int _numTileTypes, Tile* _tilesArr, sf::Vector2i _mapSize, int* _tileTypes, int* _logGrid, sf::Vector2u _tileSize)
{
...
TileMapOld::~TileMapOld() {}

void TileMapOld::render(sf::Vector2i curTile, sf::RenderWindow& win, sf::Vector2i _rendSize)
{
...
void TileMapOld::renderAll(sf::RenderWindow& win) {
	for (int y = 0; y < mapSize.y; y++) {
		for (int x = 0; x < mapSize.x; x++) {
...
void TileMapOld::renderRow(sf::Vector2i curTile, int whichRow, sf::Vector2i _rendSize, sf::RenderWindow& win)
{
	//curTile is updated in main and passed in
...
void TileMapOld::renderOne(sf::RenderWindow& win, int x, int y) {
	win.draw(getTileAt(x, y));
}

Tile TileMapOld::getTileAt(int x, int y)
{
	return tileV.at(x + y * mapSize.x);
}

Tile TileMapOld::getTileWithPoints(sf::Vector2f cords) {
